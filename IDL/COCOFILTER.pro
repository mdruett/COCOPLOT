...
;                end wavelengths, with 1.96 sd between each filter
   nlambda=n_elements(wavelengths)    ; GV can come out of case-loop
   ...
   'single': begin
      ...
      ; GV short-hand D to cast into double 
   ...
   'band': begin
      ...
      filt_int=1D/double(filt_bits[*,1]-filt_bits[*,0]+1)   ;GV moved outside loop; IDL can do it in one go
   ...
   'normal': begin
     ...
     ; GV should use n_elements() as keyword_set()
     ; may behave weirdly when provided with non-boolean values 0 and/or 1
     wavelengths_filt = double(wavelengths_filt) ; GV make them double() once
     ...
     if (n_elements(r) eq 2) then begin ; GV keyword_set(g) was typo I believe -MD yes
